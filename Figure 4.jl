...
# predictive models fail to recover Floquet structure even with abundant data.
#
# 5 panels:
#   (a) Schematic: phase-partitioned regions around a limit cycle
#   (b) Forward prediction error compounds at region boundaries
...
# ── Simulate Stuart-Landau with long trajectory ─────────────────────────────
const PW_σ  = 0.1
const PW_ω  = 2π
const PW_dt = 0.01
const PW_T_period = 2π / PW_ω   # = 1.0 s

# Ground truth Floquet
μ_true_phase, μ_true_trans = ground_truth_floquet_sl(σ=PW_σ, ω=PW_ω)
@info "Ground-truth Floquet:" μ_phase=μ_true_phase μ_trans=μ_true_trans
...
results_pw = @cached cache_path("fig4_piecewise_results.jls") begin
    ...
    # 1. Partition sweep (fixed data length = 1000 periods, varying noise)
    for noise in noise_levels
        T_sim = 1000 * PW_T_period + 1.0  # extra for transient
        t, xd, yd = simulate_stuart_landau(σ=PW_σ, ω=PW_ω, T=T_sim, dt=PW_dt,
                                            z0=sqrt(PW_σ) + 0im, noise=noise)
        # Discard transient
        n_disc = round(Int, 1.0 / PW_dt)
        xd = xd[n_disc+1:end]
        yd = yd[n_disc+1:end]
    ...
    for noise in noise_levels
        for n_periods in data_lengths
            ...
            A_nc, μ_nc = next_cycle_dmd(xd, yd, PW_dt)
    ...
    t_fwd, x_fwd, y_fwd = simulate_stuart_landau(σ=PW_σ, ω=PW_ω, T=T_sim_fwd, dt=PW_dt,
                                                    z0=sqrt(PW_σ) + 0im, noise=0.0)
    # Train on long data, predict on this segment
    T_train = 500 * PW_T_period + 1.0
    _, x_train, y_train = simulate_stuart_landau(σ=PW_σ, ω=PW_ω, T=T_train, dt=PW_dt,
                                                   z0=sqrt(PW_σ) + 0im, noise=0.005)
    n_disc = round(Int, 1.0 / PW_dt)
    x_train = x_train[n_disc+1:end]
    y_train = y_train[n_disc+1:end]
...
p_a = plot(r_lc .* cos.(θ_lc), r_lc .* sin.(θ_lc);
           lw=2, color=:black, label="Limit cycle",
           aspect_ratio=:equal, xlabel="x", ylabel="y",
           title="(A) Phase partitions",
           size=(PUB_W, PUB_H), xlims=(-0.5, 0.5), ylims=(-0.5, 0.5))
n_show = 8
boundaries_show = range(0, 2π, length=n_show + 1)
for j in 1:n_show
    θ_start = boundaries_show[j]
    θ_end = boundaries_show[j + 1]
    θ_seg = range(θ_start, θ_end, length=40)
    plot!(p_a, r_lc .* cos.(θ_seg), r_lc .* sin.(θ_seg);
          lw=4, color=partition_colors[j], alpha=0.7, label=(j <= 3 ? "Region $j" : ""))
    # Radial boundary lines
    plot!(p_a, [0, 0.45 * cos(θ_start)], [0, 0.45 * sin(θ_start)];
          lw=0.5, ls=:dash, color=:gray, label="")
end

# ── Panel (b): Forward prediction error compounds ───────────────────────────
t_fwd_ax = range(0, step=PW_dt, length=round(Int, 20 * PW_T_period / PW_dt) + 1)

p_b = plot(; xlabel="Time (periods)", ylabel="Prediction error",
             title="(B) Piecewise forward error compounds",
             size=(PUB_W, PUB_H), yscale=:log10)
for np in [4, 8, 16, 32]
    key = ("forward_error", np)
    if haskey(results_pw, key)
        errs = results_pw[key]
        t_ax = (0:length(errs)-1) .* PW_dt ./ PW_T_period
        plot!(p_b, t_ax, max.(errs, 1e-15);
...
p_c = plot(; xlabel="Number of partitions", ylabel="|μ̂_trans − μ_true|",
             title="(C) Piecewise Floquet error",
             size=(PUB_W, PUB_H), yscale=:log10, xscale=:log10)
...
for noise in noise_levels
    ...
    for np in n_partitions_sweep
        ...
        if haskey(results_pw, key)
            push!(errs, max(results_pw[key].err, 1e-10))
            push!(nps, np)
        end
    end
...
p_d = plot(; xlabel="Training periods", ylabel="|μ̂_trans − μ_true|",
             title="(D) Next-cycle model error",
             size=(PUB_W, PUB_H), yscale=:log10, xscale=:log10)
for noise in noise_levels
    errs = Float64[]
    lens = Int[]
...
hline!(p_d, [0.01]; ls=:dash, color=:gray, lw=1, label="1% error")
...
total_samples = round(Int, 1000 * PW_T_period / PW_dt)
for np in n_partitions_sweep
